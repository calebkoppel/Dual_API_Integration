...
function create_movie_search_request(api_key, movie, city, state, res){
	...
	let movie_results;
	try {
		const movie_cache = require("./cache/movies.json");
...
function received_movie_result(serialized_search_object, city, state, res){
		...
		fs.readFile("./cache/movies.json", (err, data)=>{	
			let movies = [];
			try {
			...
			movies.push(results);
			fs.writeFile("./cache/movies.json", JSON.stringify(movies, null, 4), ()=>{console.log("Movie Cached Successfully")});
		});
...
function create_beer_request(city, state, movie_results, res){
	...
	try {
		const beer_cache = require("./cache/beer.json");
...
function received_beer_result(serialized_search_object, movie_results, res){
	...
	fs.readFile("./cache/beer.json", (err, data)=>{
		let beers = [];
		try {
			...
			beers.push(...brewery);
			fs.writeFile("./cache/beer.json", JSON.stringify(beers, null, 4), ()=>{console.log("Breweries Cached Successsfully")});
	});
